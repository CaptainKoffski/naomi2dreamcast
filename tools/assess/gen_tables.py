...
from run_battery import BATTERY_VERSION  # noqa: E402 — scorer/battery bump together (§6)
...
def ranking():
    ...
    lines = ["# Portability ranking", "",
             ...
             f"All scores come from one blanket pass of the **v{BATTERY_VERSION} scorer** "
             "(§6 items 6–8 re-keys); Battery = capture provenance — the battery version that "
             "*measured* the row's sidecar (`versions.battery`), not the scorer. Sidecars "
             "missing newer keying fields score through provably conservative fallbacks "
             "(lower bounds; kb §6/§11). Main>16MB / VRAM>8MB / ARAM>2MB = informational "
