...
| Assessed | capture 2026-08-09 · battery v9 · flycast `f014a410c` · Ghidra 12.1.2_PUBLIC · MAME `59e7c0b` — fresh v9 capture (see History) |
...
Attract/demo reached: **demo** — sidecar `capture.coverage = "demo"`; attract cycle across the 10 shots: red blood-splash story transition → character-portrait story art → sky/rocket cinematic → live demo gameplay (`DEMONSTRATION` overlay, boss, EXP 11,767,380 Level 180) → sky cinematic → live demo gameplay (`DEMONSTRATION`, EXP 2,712,710 Level 181, brain boss) → black transition → live demo gameplay (`DEMONSTRATION`, Level 180, forest) → ADX publisher card → title (カラス feather art).
Screenshots kept (5 of 10):
- `assessments/evidence/karous/shot-060s.png` — story transition: red blood-splash cut
- `assessments/evidence/karous/shot-121s.png` — story art: character portrait, night scene
- `assessments/evidence/karous/shot-243s.png` — attract gameplay: `DEMONSTRATION` overlay, boss fight, EXP 11,767,380, Level 180
...
Anomalies: memory peaks reproduced byte-identically vs the prior (v8-capture) sidecar — main 32,505,920, VRAM 13,778,944, ARAM 2,097,136, `dma_high_water` 27,289,280 all unchanged. Content-volume counters shifted by 1 byte each (main `nz_total` 6,738,573 vs 6,738,574; ARAM `content_total` 1,545,307 vs 1,545,306) and `streaming.steady_mb_per_min` shifted 14.284 vs 14.287 — capture-to-capture noise, no score impact (all five axis scores unchanged: memory 100.0, streaming 62.4, guts 85.0, controls 100.0, similarity 70.0).

## 4. Memory fit (axis: 100.0)

| Region | Scored value | DC cap | u | Sub-score | Evidence / note |
|---|---|---|---|---|---|
| Main RAM (write-truth content volume, `nz_total`) | 6,738,573 | 16,777,216 | 0.402 | 100.0 | address peak 32,505,920 (`0x1F00040`, u 1.938, informational) — **fifth instance of the kb §6 item 3 shared-structure signature** · `nz_above_cap` 5,092,992 B of real changed content above the 16 MB line — the shipped DC port trimmed/restructured ~5 MB · `dma_high_water` 27,289,280 (byte-identical to illvelo's v4 figure — same-engine determinism) |
...
| ARAM (content volume, fill-excluded, `content_total`) | 1,545,307 | 2,097,152 | 0.737 | 100.0 | address peak 2,097,136 — 16 B under the cap, **fits even address-keyed**; matches illvelo's v4 ARAM peak exactly (the Milestone engine loads a just-under-2-MiB sound budget, DC-sized by construction) |
...
re-read ratio 0.7713 · steady-state 14.284 MB/min (`short_window: false`) ·
...
| v9 | 2026-08-09 | 85.0 S | ranking-groom chunk 2: fresh v9 capture, provenance v8→v9 (scoring keys unchanged) |
